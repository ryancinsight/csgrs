...
impl<S: Clone + Debug + Send + Sync> CSG<S> {
    ...
    pub fn extrude_vector(&self, direction: Vector3<Real>) -> CSG<S> {
        ...
        // Collect the new 3D polygons generated by extrusion using iterator patterns with parallel processing:
        #[cfg(feature = "parallel")]
        let new_polygons: Vec<Polygon<S>> = {
            if self.geometry.len() > 100 {
                use rayon::prelude::*;
                ...
                self.geometry
                    ...
                    .flat_map(|geom| {
                        ...
                        extrude_geometry(geom, direction, &self.metadata, &mut polygons);
                        polygons
                    })
                    .collect()
            } else {
...
fn extrude_geometry<S: Clone + Send + Sync>(
    ...
    match geom {
        geo::Geometry::Polygon(poly) => {
            ...
            // bottom triangles using iterator patterns
            ...
            // top triangles using iterator patterns
            tris.iter().for_each(|tri| {
        ...
        geo::Geometry::LineString(ls) => {
            // extrude line strings into side surfaces using iterator patterns
            let coords: Vec<_> = ls.coords_iter().collect();

            // Use iterator patterns with parallel processing for large line strings
            #[cfg(feature = "parallel")]
            let line_polygons: Vec<Polygon<S>> = {
                if coords.len() > 1000 {
                    use rayon::prelude::*;
            ...
            #[cfg(not(feature = "parallel"))]
            let line_polygons: Vec<Polygon<S>> = coords
                .windows(2)
                .map(|window| {
                    let c_i = window[0];
                    let c_j = window[1];
            ...
            out_polygons.extend(line_polygons);
        },
