...
impl<S: Clone + Debug + Send + Sync> CSG<S> {
    ...
    pub fn to_obj(&self, object_name: &str) -> String {
        ...
        obj_content.push_str(&format!("# Object: {}\n", object_name));
        ...
        for geom in &self.geometry.0 {
            match geom {
                geo::Geometry::Polygon(poly2d) => {
                    self.add_2d_polygon_to_obj(poly2d, &mut vertices, &mut normals, &mut faces);
                }
    ...
    /// # }
    /// ```
    pub fn write_obj<W: Write>(&self, writer: &mut W, object_name: &str) -> std::io::Result<()> {
    ...
    /// # }
    /// ```
    pub fn from_obj<R: BufRead>(reader: R, metadata: Option<S>) -> std::io::Result<CSG<S>> {
        let mut vertices = Vec::new();
        let mut normals = Vec::new();
        ...
        for line_result in reader.lines() {
            ...
            match parts[0] {
                ...
                "f" => {
                    ...
                    if parts.len() >= 4 {
                        let face_vertices = Self::parse_obj_face(&parts[1..], &vertices, &normals)?;
    ...
    fn add_2d_polygon_to_obj(
        ...
        let normal_idx = Self::add_unique_normal(normals, normal);
        ...
        for triangle in triangles_2d {
            ...
            for point in triangle {
                ...
                let vertex_idx = Self::add_unique_vertex(vertices, vertex_3d);
                face_indices.push((vertex_idx, normal_idx));
            }
