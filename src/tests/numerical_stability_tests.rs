...
mod numerical_stability {
    ...
    fn test_csg_numerical_stability() {
        ...
        assert!(union_result.polygons.len() > 0, "Union should produce valid geometry");

        // Test difference operation
        let difference_result = base_mesh.difference(&offset_mesh);
        assert!(difference_result.polygons.len() > 0, "Difference should produce valid geometry");

        // Test intersection operation
        let intersection_result = base_mesh.intersection(&offset_mesh);
        assert!(intersection_result.polygons.len() > 0, "Intersection should produce valid geometry");
    }
