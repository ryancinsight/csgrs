...
fn main() {
    ...
    for (vertex_idx, neighbors) in &adjacency_map {
        ...
        if *vertex_idx < 5 { // Show first few for demonstration
            println!("   Vertex {}: {} neighbors -> {:?}", 
                     vertex_idx, neighbors.len(), 
    ...
    for &vertex_idx in adjacency_map.keys().take(10) {
        let (valence, regularity) = csgrs::geometry::vertex::Vertex::analyze_connectivity_with_index(
            vertex_idx,
            &adjacency_map
        );
        regularity_samples.push(regularity);
        
        if vertex_idx < 3 { // Show first few
            println!("   Vertex {}: valence={}, regularity={:.3}", vertex_idx, valence, regularity);
        }
    }
    
    let avg_regularity: f64 = regularity_samples.iter().sum::<f64>() / regularity_samples.len() as f64;
    ...
    println!("   Original test vertex position: ({:.3}, {:.3}, {:.3})", 
    ...
    println!("   After weak smoothing (λ=0.1): ({:.3}, {:.3}, {:.3})", 
             weak_pos.x, weak_pos.y, weak_pos.z);
    println!("   After strong smoothing (λ=0.3): ({:.3}, {:.3}, {:.3})", 
    ...
    assert!(strong_change > weak_change, "Stronger smoothing should cause more change");
    ...
    let refined = tessellated.adaptive_refine(0.5, 2.0);
