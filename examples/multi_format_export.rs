...
//!
//! This example demonstrates exporting CSG objects to multiple 3D file formats:
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create various CSG objects to demonstrate OBJ export
    ...
    // 2. Sphere
    let sphere: CSG<()> = CSG::sphere(15.0, 32, 16, None);
    export_to_obj(&sphere, "sphere", "Sphere with 15mm radius")?;

    // 3. Cylinder
    let cylinder: CSG<()> = CSG::cylinder(8.0, 25.0, 24, None);
    export_to_obj(&cylinder, "cylinder", "Cylinder: 8mm radius, 25mm height")?;

    // 4. Complex boolean operation: cube with spherical cavity
    let cube_large: CSG<()> = CSG::cube(30.0, None).center();
    let sphere_cavity: CSG<()> = CSG::sphere(12.0, 24, 12, None).translate(5.0, 5.0, 0.0);
    let cube_with_cavity = cube_large.difference(&sphere_cavity);
    export_to_obj(&cube_with_cavity, "cube_with_cavity", "30mm cube with 12mm spherical cavity")?;

    // 5. Union operation: cube + sphere
    let cube_small: CSG<()> = CSG::cube(16.0, None).center();
    let sphere_union: CSG<()> = CSG::sphere(10.0, 20, 10, None).translate(8.0, 8.0, 8.0);
    let union_object = cube_small.union(&sphere_union);
    export_to_obj(&union_object, "cube_sphere_union", "Union of 16mm cube and 10mm sphere")?;

    // 6. Intersection operation
    let cube_intersect: CSG<()> = CSG::cube(25.0, None).center();
    let sphere_intersect: CSG<()> = CSG::sphere(15.0, 24, 12, None).translate(5.0, 5.0, 0.0);
    let intersection_object = cube_intersect.intersection(&sphere_intersect);
    export_to_obj(&intersection_object, "cube_sphere_intersection", "Intersection of cube and sphere")?;

    // 7. More complex shape: cube with cylindrical hole
    let cube_base: CSG<()> = CSG::cube(40.0, None).center();
    let hole_cylinder: CSG<()> = CSG::cylinder(6.0, 50.0, 16, None)
        .rotate(90.0, 0.0, 0.0)  // Rotate to align with X-axis
        .translate(0.0, 0.0, 0.0);
    let cube_with_hole = cube_base.difference(&hole_cylinder);
    export_to_obj(&cube_with_hole, "cube_with_hole", "40mm cube with 12mm diameter hole")?;
...
fn export_to_obj(csg: &CSG<()>, name: &str, description: &str) -> Result<(), Box<dyn std::error::Error>> {
    // Export OBJ format
    #[cfg(feature = "obj-io")]
    {
        use std::fs::File;
        ...
        let normal_count = obj_content.lines().filter(|line| line.starts_with("vn ")).count();
        
        println!("  OBJ Stats: {} vertices, {} faces, {} normals", vertex_count, face_count, normal_count);
    }
    ...
    #[cfg(feature = "ply-io")]
    {
        ...
        let vertex_count = ply_content.lines().filter(|line| !line.starts_with("ply") && 
                                                             ...
                                                             !line.starts_with("3 ") &&
                                                             !line.trim().is_empty()).count();
        let face_count = ply_content.lines().filter(|line| line.starts_with("3 ")).count();
        
        println!("  PLY Stats: {} vertices, {} triangles with normals", vertex_count, face_count);
    }
    ...
    #[cfg(feature = "amf-io")]
    {
        ...
        println!("  AMF Stats: {} vertices, {} triangles (XML format)", vertex_count, triangle_count);
    }
...
mod tests {
    ...
    fn test_obj_content_format() {
        let sphere: CSG<()> = CSG::sphere(5.0, 8, 4, None);  // Low res for testing
        ...
        #[cfg(feature = "obj-io")]
        {
            ...
            let vertex_lines: Vec<_> = lines.iter().filter(|line| line.starts_with("v ")).collect();
            ...
            let normal_lines: Vec<_> = lines.iter().filter(|line| line.starts_with("vn ")).collect();
            assert!(!normal_lines.is_empty());
            
            // Should have faces (format: f v1//n1 v2//n2 v3//n3)
            let face_lines: Vec<_> = lines.iter().filter(|line| line.starts_with("f ")).collect();
    ...
    fn test_boolean_operations_obj_export() {
        ...
        let cube: CSG<()> = CSG::cube(10.0, None);
        let sphere: CSG<()> = CSG::sphere(6.0, 8, 4, None);
    ...
    fn test_ply_export() {
        ...
        #[cfg(feature = "ply-io")]
        {
            ...
            let face_lines: Vec<_> = data_lines.iter()
    ...
    fn test_ply_format_structure() {
        ...
        #[cfg(feature = "ply-io")]
        {
            ...
            let vertex_line = lines.iter().find(|line| line.starts_with("element vertex")).unwrap();
            let face_line = lines.iter().find(|line| line.starts_with("element face")).unwrap();
            
            let vertex_count: usize = vertex_line.split_whitespace().nth(2).unwrap().parse().unwrap();
            let face_count: usize = face_line.split_whitespace().nth(2).unwrap().parse().unwrap();
            ...
            let vertex_data_lines: Vec<_> = data_lines.iter()
                .filter(|line| {
            ...
            let face_data_lines: Vec<_> = data_lines.iter()
                .filter(|line| line.starts_with("3 "))
                .collect();
    ...
    fn test_amf_export() {
        ...
        #[cfg(feature = "amf-io")]
        {
            ...
            // Check metadata
            assert!(amf_content.contains("<metadata type=\"producer\">csgrs library</metadata>"));
            assert!(amf_content.contains("<metadata type=\"cad\">Constructive Solid Geometry</metadata>"));
    ...
    fn test_amf_with_color() {
        ...
        #[cfg(feature = "amf-io")]
        {
            let amf_content = sphere.to_amf_with_color("red_sphere", "millimeter", (1.0, 0.0, 0.0));
    ...
    fn test_amf_xml_structure() {
        let cube: CSG<()> = CSG::cube(8.0, None);
